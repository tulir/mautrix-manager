...
@routes.view("/mx-puppet-instagram/{path:.+}")
...
@routes.get("/mx-puppet-instagram")
...
def init(cfg: Config, app: web.Application) -> None:
    ...
    config = cfg
    secret = cfg["bridges.mx-puppet-instagram.secret"]
    if secret:
        host = URL(cfg["bridges.mx-puppet-instagram.url"])
